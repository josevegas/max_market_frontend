...
import { SubCategoria, Categoria } from '../../models/catalogo.model';
import { SubCategoriaService, CategoriaService } from '../../services/productos.service';

@Component({
  standalone: true,
  selector: 'app-sub-categoria-lista',
  imports: [
  ...
  templateUrl: './sub-categoria-lista.html',
})
export class SubCategoriaLista implements OnInit {
  private readonly svc = inject(SubCategoriaService);
  private readonly padreSvc = inject(CategoriaService);
  private readonly msg = inject(MessageService);
  private readonly confirm = inject(ConfirmationService);

  readonly cargando = signal(false);
  readonly guardando = signal(false);
  readonly items = signal<SubCategoria[]>([]);
  readonly verInactivos = signal(false);

  /** Categoría disponibles para el selector y para resolver el nombre en
   *  la tabla: la API devuelve el id, no el nombre. */
  readonly padres = signal<Categoria[]>([]);
  ...
  readonly editando = signal<SubCategoria | null>(null);
  nombre = '';
  codigo = '';
  ...
  editar(item: SubCategoria): void {
    this.editando.set(item);
    this.nombre = item.nombre;
    this.codigo = item.codigo ?? '';
    this.padreId = item.categoria_id;
    this.dialogoAbierto.set(true);
  }
  ...
  guardar(): void {
    ...
    peticion.subscribe({
      next: () => {
        ...
        this.msg.add({
          severity: 'success',
          summary: enEdicion ? 'Sub categoría actualizada' : 'Sub categoría creada',
          life: 2500,
        });
        this.cargar();
      },
  ...
  darDeBaja(item: SubCategoria): void {
    this.confirm.confirm({
      ...
      accept: () =>
        this.svc.desactivar(item.id).subscribe({
          next: () => {
            this.msg.add({ severity: 'success', summary: 'Sub categoría dada de baja', life: 2500 });
            this.cargar();
          },
          error: (e: AppError) =>
            this.msg.add({ severity: 'error', summary: 'Error', detail: e.message }),
